...
class SignUpViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        avatarButton.imageEdgeInsets = UIEdgeInsets(top: 10, left: 20, bottom: 10, right: 20)    }
    ...
    @IBAction func avatarButtonClicked(_ sender: Any) {
        ...
        viewCenterX.constant = 0
    }
    @IBAction func personFillButtonCliked(_ sender: Any) {
        avatarButton.setImage(UIImage(systemName: "person.fill"), for: .normal)
        viewCenterX.constant = -100000000
        UserDefaults.standard.set("person.fill", forKey: "avatarImageName")
    }
    @IBAction func personButtonClicked(_ sender: Any) {
        avatarButton.setImage(UIImage(systemName: "person"), for: .normal)
        viewCenterX.constant = -100000000
        UserDefaults.standard.set( "person", forKey: "avatarImageName")

    }
    
    @IBAction func personCircleButtonClicked(_ sender: Any) {
        avatarButton.setImage(UIImage(systemName: "person.circle"), for: .normal)
        viewCenterX.constant = -100000000
        UserDefaults.standard.set("person.circle", forKey: "avatarImageName")
    }
              ...
              if error != nil {
                  ...
                  Auth.auth().createUser(withEmail: email, password: password) { (result, error) in
                      if error != nil {
                          ...
                          db.collection("users").document("\(email)").setData([             "uid": result?.uid , "age": self.age,
                                                                                            "username": username,
